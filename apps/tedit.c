...
void textedit_insert_line(int line_num, const char *text) {
  if (line_num < 0 || line_num > current_line_count ||
      current_line_count >= TEXT_EDIT_MAX_LINES) {
...
void textedit_set_format(int line_num, text_color_t color,
                         text_align_t alignment, uint8_t bold, uint8_t italic,
                         uint8_t underline) {
  if (line_num < 0 || line_num >= current_line_count) {
    print_error("Invalid line number");
    return;
...
void textedit_show() {
  ...
  for (int i = 0; i < current_line_count; i++) {
    ...
    case ALIGN_JUSTIFY:
      print(text, term_color);
      for (int s = len; s < TEXT_EDIT_LINE_LENGTH; s++)
        print(" ", term_color);
      break;
    }
    print("\n", WHITE);
...
void textedit_save(const char *filename) {
  ...
  for (int i = 0; i < current_line_count; i++) {
    ptr +=
        sprintf(ptr, "%d,%d,%d,%d,%d|", text_buffer[i].format.color,
                text_buffer[i].format.alignment, text_buffer[i].format.bold,
                text_buffer[i].format.italic, text_buffer[i].format.underline);
    const char *text = text_buffer[i].text;
    while (*text) {
      *ptr++ = *text++;
    }
    ...
    if (buffer[i] == '\n' || buffer[i] == '\0') {
      if (line_start < &buffer[i]) {
        ...
        if (format_end) {
          ...
          if (sscanf(line_start, "%d,%d,%d,%d,%d", &color, &alignment, &bold,
                     &italic, &underline) == 5) {
          ...
          if (text_len > 0) {
            int copy_len = (text_len > TEXT_EDIT_LINE_LENGTH - 1)
                               ? TEXT_EDIT_LINE_LENGTH - 1
                               : text_len;
...
void textedit_search(const char *pattern) {
  ...
  for (int i = 0; i < current_line_count; i++) {
    ...
    if (pos) {
      ...
      print_dec(i + 1, CYAN);
      print(": ", WHITE);
      print(text_buffer[i].text,
            get_terminal_color(text_buffer[i].format.color));
      print("\n", WHITE);
    }
  }
  if (!found) {
