...
int spsheet_parse_cell(const char* cell_ref, int* row, int* col) {
    if (strlen(cell_ref) < 2) return 0;
    char col_char = s_toupper(cell_ref[0]);
    if (col_char < 'A' || col_char > 'A' + SPREADSHEET_COLS - 1) return 0;
    *col = col_char - 'A';
    *row = atoi(cell_ref + 1) - 1;
    if (*row < 0 || *row >= SPREADSHEET_ROWS) return 0;
    return 1;
}
    ...
    for (int i = 0; i <= bytes_read; i++) {
        if (buffer[i] == '\n' || buffer[i] == '\0') {
            if (row < SPREADSHEET_ROWS && col < SPREADSHEET_COLS) {
                ...
                if (line_len > 0) {
                    int copy_len = (line_len > CELL_VALUE_LENGTH-1) ? CELL_VALUE_LENGTH-1 : line_len;
            ...
            if (buffer[i] == '\0' || row >= SPREADSHEET_ROWS) break;
        }
    }
