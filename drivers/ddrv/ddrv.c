...
bool disk_read_multiple_sectors(uint32_t lba, uint8_t num_sectors, uint8_t* buffer) {
...
bool disk_write_multiple_sectors(uint32_t lba, uint8_t num_sectors, uint8_t* buffer) {
    for (int i = 0; i < num_sectors; i++) {
...
disk_drive_t* disk_get_drive(uint8_t index) { return (index == 0) ? &drives[0] : NULL; }
...
void disk_print_info(void) {
    ...
    print("Model: ", WHITE); print(drive->info.model, WHITE); print("\n", WHITE);
    print("Size: ", WHITE); print_dec(drive->info.capacity_mb, GREEN); print(" MB\n", WHITE);
    print("Sectors: ", WHITE); print_dec(drive->info.total_sectors, GREEN); print("\n", WHITE);
}
