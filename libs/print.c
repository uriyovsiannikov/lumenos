...
void scroll_screen() {
  ...
  memmove((void *)vga_buffer, (void *)(vga_buffer + VGA_WIDTH),
...
void putchar(char c, uint8_t color) {
  if (c == '\n') {
    ...
    do {
      putchar(' ', color);
    } while (cursor_x % 4 != 0 && cursor_x < VGA_WIDTH);
    return;
  } else if (c >= ' ') {
...
void print_success(const char *msg) { print(msg, LIGHT_GREEN); }
void print_info(const char *msg) { print(msg, LIGHT_CYAN); }
void print_warning(const char *msg) { print(msg, YELLOW); }
