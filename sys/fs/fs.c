...
int fs_create(const char *filename, bool is_dir, uint32_t parent_dir,
              bool isSilent) {
  if (!disk_read_sector(FS_ROOT_DIR_SECTOR, (uint8_t *)fs_files)) {
    if (!isSilent)
  ...
  if (parent_dir >= FS_MAX_FILES || !fs_files[parent_dir].is_used ||
      !fs_files[parent_dir].is_dir) {
    if (!isSilent)
  ...
  for (int i = 0; i < FS_MAX_FILES; i++) {
    if (fs_files[i].is_used && fs_files[i].parent_dir == parent_dir &&
      ...
      if (!isSilent)
        print_error("File/directory already exists\n");
      return -1;
    }
  }
  for (int i = 1; i < FS_MAX_FILES; i++) {
    if (!fs_files[i].is_used) {
      ...
      if (!disk_write_sector(FS_ROOT_DIR_SECTOR, (uint8_t *)fs_files)) {
        if (!isSilent)
          print_error("Failed to save filesystem\n");
  ...
  if (!isSilent)
    print_error("Maximum files reached\n");
  return -1;
}
int fs_write(const char *filename, const void *data, uint32_t size,
  ...
  for (uint32_t sector = FS_DATA_START_SECTOR;
       sector < 1000 && sectors_found < sectors_needed; sector++) {
    if (fs_fat[sector] == 0) {
...
int fs_read(const char *filename, void *buffer, uint32_t max_size,
  ...
  while (bytes_read < bytes_to_read && current_sector != 0xFFFF) {
    ...
    if (bytes_in_sector > FS_SECTOR_SIZE)
      bytes_in_sector = FS_SECTOR_SIZE;
...
int fs_list(uint32_t dir_id) {
  ...
  if (dir_id >= FS_MAX_FILES || !fs_files[dir_id].is_used ||
      !fs_files[dir_id].is_dir) {
...
uint32_t fs_get_current_dir(void) { return fs_current_dir; }
...
int fs_info(const char *name, uint32_t parent_dir) {
    ...
    if (!file) {
        print_error("File/directory not found: ");
        print(name, WHITE);
        print("\n", WHITE);
        return -1;
    }
    ...
    print(" Name: ", WHITE);
    print(file->name, file->is_dir ? CYAN : GREEN);
    print("\n", WHITE);
    print(" Type: ", WHITE);
    if (file->is_dir) {
        print("Directory\n", CYAN);
    ...
    print(" Size: ", WHITE);
    if (file->is_dir) {
        print("Directory (no size)\n", WHITE);
    } else {
        print_dec(file->size, GREEN);
        print(" bytes (", WHITE);
        if (file->size < 1024) {
            print_dec(file->size, GREEN);
            print(" B)\n", WHITE);
        } else if (file->size < 1024 * 1024) {
            print_dec(file->size / 1024, GREEN);
            print(" KB (", WHITE);
            print_dec(file->size, GREEN);
            print(" bytes)\n", WHITE);
        } else {
            print_dec(file->size / (1024 * 1024), GREEN);
            print(" MB (", WHITE);
            print_dec(file->size, GREEN);
            print(" bytes)\n", WHITE);
        }
    }
    print(" Permissions: ", WHITE);
    if (file->is_readonly) {
        print("Read-Only\n", RED);
    } else {
        print("Read/Write\n", GREEN);
    ...
    print(" Index: ", WHITE);
    print_dec(file_index, YELLOW);
    print("\n", WHITE);
    print(" Parent Directory ID: ", WHITE);
    print_dec(file->parent_dir, YELLOW);
    print(" (", WHITE);
    if (file->parent_dir < FS_MAX_FILES && fs_files[file->parent_dir].is_used) {
        print(fs_files[file->parent_dir].name, CYAN);
    } else {
        print("unknown", GRAY);
    }
    print(")\n", WHITE);
    print(" First Sector: ", WHITE);
    if (file->first_sector == 0xFFFF) {
        print("None (empty file)\n", GRAY);
    } else {
        print_hex(file->first_sector, YELLOW);
        print("\n", WHITE);
        if (!file->is_dir && file->first_sector != 0xFFFF) {
            print(" Sector Chain: ", WHITE);
            uint32_t current_sector = file->first_sector;
            int sector_count = 0;
            uint8_t sector_buffer[FS_SECTOR_SIZE];
            if (disk_read_sector(FS_FAT_SECTOR, (uint8_t *)fs_fat)) {
                print("[", WHITE);
                while (current_sector != 0xFFFF) {
                    if (sector_count > 0) print(" -> ", WHITE);
                    print_hex(current_sector, YELLOW);
                    sector_count++;
                    if (current_sector >= sizeof(fs_fat)/sizeof(fs_fat[0])) {
                        print(" (FAT error!)", RED);
                        break;
                    }
                    current_sector = fs_fat[current_sector];
                    if (sector_count > 1000) {
                        print(" ... (too long)", GRAY);
                        break;
                    }
                }
                print("]\n", WHITE);
                print(" Sectors used: ", WHITE);
                print_dec(sector_count, GREEN);
                print("\n", WHITE);
                print(" Disk space used: ", WHITE);
                print_dec(sector_count * FS_SECTOR_SIZE, GREEN);
                print(" bytes\n", WHITE);
            }
        }
    }
    ...
    if (file->is_dir) {
        ...
        print(" Contents: ", WHITE);
        print_dec(dir_count, CYAN);
        print(" directories, ", WHITE);
        print_dec(file_count, GREEN);
        print(" files\n", WHITE);
        print(" Total items: ", WHITE);
        print_dec(dir_count + file_count, YELLOW);
        print("\n", WHITE);
    }
    return 0;
}
