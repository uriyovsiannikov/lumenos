...
char get_char_from_scancode(uint8_t scancode) {
  ...
  switch (scancode) {
  case 0x02:
    c = shift_pressed ^ caps_lock ? '!' : '1';
    break;
  case 0x03:
    c = shift_pressed ^ caps_lock ? '@' : '2';
    break;
  case 0x04:
    c = shift_pressed ^ caps_lock ? '#' : '3';
    break;
  case 0x05:
    c = shift_pressed ^ caps_lock ? '$' : '4';
    break;
  case 0x06:
    c = shift_pressed ^ caps_lock ? '%' : '5';
    break;
  case 0x07:
    c = shift_pressed ^ caps_lock ? '^' : '6';
    break;
  case 0x08:
    c = shift_pressed ^ caps_lock ? '&' : '7';
    break;
  case 0x09:
    c = shift_pressed ^ caps_lock ? '*' : '8';
    break;
  case 0x0A:
    c = shift_pressed ^ caps_lock ? '(' : '9';
    break;
  case 0x0B:
    c = shift_pressed ^ caps_lock ? ')' : '0';
    break;
  case 0x0C:
    c = shift_pressed ^ caps_lock ? '_' : '-';
    break;
  case 0x0D:
    c = shift_pressed ^ caps_lock ? '+' : '=';
    break;
  case 0x10:
    c = (shift_pressed ^ caps_lock) ? 'Q' : 'q';
    break;
  case 0x11:
    c = (shift_pressed ^ caps_lock) ? 'W' : 'w';
    break;
  case 0x12:
    c = (shift_pressed ^ caps_lock) ? 'E' : 'e';
    break;
  case 0x13:
    c = (shift_pressed ^ caps_lock) ? 'R' : 'r';
    break;
  case 0x14:
    c = (shift_pressed ^ caps_lock) ? 'T' : 't';
    break;
  case 0x15:
    c = (shift_pressed ^ caps_lock) ? 'Y' : 'y';
    break;
  case 0x16:
    c = (shift_pressed ^ caps_lock) ? 'U' : 'u';
    break;
  case 0x17:
    c = (shift_pressed ^ caps_lock) ? 'I' : 'i';
    break;
  case 0x18:
    c = (shift_pressed ^ caps_lock) ? 'O' : 'o';
    break;
  case 0x19:
    c = (shift_pressed ^ caps_lock) ? 'P' : 'p';
    break;
  case 0x1A:
    c = shift_pressed ^ caps_lock ? '{' : '[';
    break;
  case 0x1B:
    ...
    break;
  case 0x1E:
    c = (shift_pressed ^ caps_lock) ? 'A' : 'a';
    break;
  case 0x1F:
    c = (shift_pressed ^ caps_lock) ? 'S' : 's';
    break;
  case 0x20:
    c = (shift_pressed ^ caps_lock) ? 'D' : 'd';
    break;
  case 0x21:
    c = (shift_pressed ^ caps_lock) ? 'F' : 'f';
    break;
  case 0x22:
    c = (shift_pressed ^ caps_lock) ? 'G' : 'g';
    break;
  case 0x23:
    c = (shift_pressed ^ caps_lock) ? 'H' : 'h';
    break;
  case 0x24:
    c = (shift_pressed ^ caps_lock) ? 'J' : 'j';
    break;
  case 0x25:
    c = (shift_pressed ^ caps_lock) ? 'K' : 'k';
    break;
  case 0x26:
    c = (shift_pressed ^ caps_lock) ? 'L' : 'l';
    break;
  case 0x27:
    c = shift_pressed ^ caps_lock ? ':' : ';';
    break;
  ...
  case 0x2B:
    ...
    break;
  case 0x2C:
    c = (shift_pressed ^ caps_lock) ? 'Z' : 'z';
    break;
  case 0x2D:
    c = (shift_pressed ^ caps_lock) ? 'X' : 'x';
    break;
  case 0x2E:
    c = (shift_pressed ^ caps_lock) ? 'C' : 'c';
    break;
  case 0x2F:
    c = (shift_pressed ^ caps_lock) ? 'V' : 'v';
    break;
  case 0x30:
    c = (shift_pressed ^ caps_lock) ? 'B' : 'b';
    break;
  case 0x31:
    c = (shift_pressed ^ caps_lock) ? 'N' : 'n';
    break;
  case 0x32:
    c = (shift_pressed ^ caps_lock) ? 'M' : 'm';
    break;
  case 0x33:
    c = shift_pressed ^ caps_lock ? '<' : ',';
    break;
...
bool is_key_buffer_empty(void) { return key_buffer_start == key_buffer_end; }
...
bool key_available(void) { return !is_key_buffer_empty(); }
void (*get_keyboard_handler(void))(void) { return current_handler; }
void set_keyboard_handler(void (*handler)(void)) { current_handler = handler; }
