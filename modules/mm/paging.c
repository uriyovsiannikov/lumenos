...
bool paging_map_page(uint32_t virtual_addr, uint32_t physical_addr,
                     uint32_t flags) {
...
void paging_print_info(void) {
  ...
  print("ACTIVE\n", GREEN);
  print("CR0.PG: ", WHITE);
  print((cr0 & 0x80000000) ? "ENABLED\n" : "DISABLED\n", CYAN);
  print("CR3:    ", WHITE);
  print_hex(cr3, CYAN);
  print("\n", WHITE);
  print("Physical Memory: ", WHITE);
  print_dec(phys_mem.used_pages * 4, CYAN);
  print(" KB / ", WHITE);
  print_dec(phys_mem.total_pages * 4, CYAN);
  print(" KB used\n", WHITE);
  print("Virtual Space:   ", WHITE);
  print_dec(initialized_tables * 4, CYAN);
  print(" MB / 64 MB mapped\n", WHITE);
  print("Page Tables:     ", WHITE);
  print_dec(initialized_tables, CYAN);
...
void paging_run_tests(void) {
  ...
  if (page) {
    ...
    print("  Virtual: ", WHITE);
    print_hex((uint32_t)page, CYAN);
    print(" -> Physical: ", WHITE);
    print_hex(paging_get_physical_address((uint32_t)page), CYAN);
    print("\n", WHITE);
  ...
  if (success) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      print_dec(i, CYAN);
      print(": ", WHITE);
      print_hex((uint32_t)pages[i], CYAN);
      print(" -> ", WHITE);
      print_hex(paging_get_physical_address((uint32_t)pages[i]), CYAN);
      print("\n", WHITE);
...
void paging_invalidate_page(uint32_t addr) {
  asm volatile("invlpg (%0)" ::"r"(addr) : "memory");
}
void paging_map_range(uint32_t vstart, uint32_t pstart, size_t pages,
                      uint32_t flags) {
...
size_t paging_get_free_pages(void) {
  return phys_mem.total_pages - phys_mem.used_pages;
}
uint32_t paging_virtual_to_physical(uint32_t virt) {
  return paging_get_physical_address(virt);
}
bool paging_is_mapped(uint32_t virt) {
  return paging_get_physical_address(virt) != 0;
}
...
void paging_demo(void) {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    if (demo_pages[i]) {
      print("  Allocated page ", WHITE);
      print_dec(i, CYAN);
      print(": VA=", WHITE);
      print_hex((uint32_t)demo_pages[i], GREEN);
      print(" PA=", WHITE);
      print_hex(paging_get_physical_address((uint32_t)demo_pages[i]), GREEN);
      print("\n", WHITE);
    }
  }
  ...
  if (paging_map_page(custom_va, custom_pa, PAGE_PRESENT | PAGE_READ_WRITE)) {
    print("  Mapped: VA=", WHITE);
    print_hex(custom_va, GREEN);
    print(" -> PA=", WHITE);
    print_hex(custom_pa, GREEN);
    print("\n", WHITE);
  ...
  for (int i = 0; i < 5; i++) {
    if (demo_pages[i]) {
      ...
      print(still_mapped ? "FAIL (still mapped)" : "OK",
            still_mapped ? RED : GREEN);
      print("\n", WHITE);
    }
  }
  ...
  if (multi_page) {
    print("  Allocated 4 pages: VA=", WHITE);
    print_hex((uint32_t)multi_page, GREEN);
    print(" Size=16KB\n", WHITE);
  ...
  print_dec(paging_get_used_pages(), CYAN);
  print(" / ", WHITE);
  print_dec(phys_mem.total_pages, CYAN);
  print("\n", WHITE);
  print("  Memory used: ", WHITE);
  print_dec(paging_get_used_pages() * 4, CYAN);
  print(" KB\n", WHITE);
  for (int i = 0; i < 5; i++) {
    if (demo_pages[i]) {
