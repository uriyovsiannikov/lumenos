...
int exec_load(const char* filename) {
    ...
    char buffer[512];
    int bytes_read = fs_read((char*)filename, buffer, sizeof(buffer), current_dir_id);
...
int execute_basic_line(const char* line) {
    ...
    if (strcmp(command, "PRINT") == 0) {
        ...
        *((uint8_t*)addr) = value;
        ...
        if (strcmp(arg1, "red") == 0) change_color(RED);
        else if (strcmp(arg1, "green") == 0) change_color(GREEN);
        else if (strcmp(arg1, "blue") == 0) change_color(BLUE);
        else if (strcmp(arg1, "cyan") == 0) change_color(CYAN);
        else if (strcmp(arg1, "magenta") == 0) change_color(MAGENTA);
        else if (strcmp(arg1, "yellow") == 0) change_color(YELLOW);
        else if (strcmp(arg1, "white") == 0) change_color(WHITE);
        ...
        if (program_lines > 0) {
            ...
            for (int i = 0; i < program_lines; i++) {
                if (execute_basic_line(program[i].code) == -1) break;
            }
            clear_basic_memory();
        } else {
            print("No program in memory\n", YELLOW);
...
void basic_run(const char* program_text) {
    ...
    for (int i = 0; i < line_count; i++) {
        if (strlen(lines[i]) > 0) {
            if (execute_basic_line(lines[i]) == -1) break;
        }
    }
    clear_basic_memory();
}
void create_basic_executable(const char* filename, const char* basic_code) {
    ...
    fs_write((char*)filename, buffer, sizeof(lumen_header_t) + strlen(basic_code), fs_current_dir);
