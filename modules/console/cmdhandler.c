...
void process_command(const char *cmd) {
    ...
    if (strcmp(cmd, "help") == 0) {
	...
	else if (strcmp(cmd, "echo") == 0) print_info("Usage: echo <text> OR echo $<(all) sysvar name>");
	...
	else if (strcmp(cmd, "ioport") == 0) print_info("Usage: ioport <hex> [r|w <value>]");
		...
		if (sscanf(cmd + 7, "%hx %c %hhx", &port, &op, &value) >= 2) {
			if (op == 'r') {
				...
				print("Port 0x", WHITE); print_hex(port, CYAN);
				print(" = 0x", WHITE); print_hex(res, GREEN); print("\n", WHITE);
			} 
			else if (op == 'w') {
				outb(port, value);
				print("Wrote 0x", WHITE); print_hex(value, GREEN);
				print(" to 0x", WHITE); print_hex(port, CYAN); print("\n", WHITE);
			}
		} else {
		...
		print_info("\n  Examples:");
		print("\n    hd 0x7C00             - Dump 16 bytes at bootloader address", WHITE);
		print("\n    hd -w -n 32 0xB8000   - Dump 32 words (64 bytes) of video memory", WHITE);
		print("\n    hd -d -nos 0x100000 4 - Dump 4 dwords without ASCII view", WHITE);
		...
		while (token != NULL && !parse_error) {
			...
			if (!parse_error) token = strtok(NULL, " ");
		}
		if (parse_error) {
		...
		if (verbose) {
			...
			print("  Unit size: ", WHITE); print_dec(unit_size, CYAN); 
			print(unit_size == 1 ? " byte\n" : " bytes\n", WHITE);
			print("  Count:    ", WHITE); print_dec(count, CYAN); print("\n", WHITE);
			print("  Address:  ", WHITE); print_hex(addr, CYAN); print("\n", WHITE);
			print("  Mode:     ", WHITE); 
			print(physical ? "Physical" : "Virtual", CYAN); print("\n", WHITE);
			return;
		}
		if (addr == 0) {
		...
		print("Dump ", WHITE); print_dec(count, CYAN); 
		print(unit_size == 1 ? " bytes" : unit_size == 2 ? " words" : " dwords", WHITE);
		print(" at 0x", WHITE); print_hex(addr, CYAN); 
		print(" (", WHITE); print_dec(total_bytes, CYAN); print(" bytes):\n", WHITE);
		for (uint32_t offset = 0; offset < total_bytes; offset += 16) {
			uint32_t current_addr = addr + offset;
			print_hex(current_addr, GRAY); print(": ", WHITE);
			for (uint32_t i = 0; i < 16 && (offset + i) < total_bytes; i += unit_size) {
				switch (unit_size) {
					case 1: {
			...
			if (unit_size > 1) {
				uint32_t spaces = (16 - (total_bytes - offset)) * (unit_size == 2 ? 5 : 3);
				for (uint32_t i = 0; i < spaces; i++) putchar(' ', WHITE);
			}
			if (show_ascii) {
				print(" ", WHITE);
		...
		print(" ##### \n",WHITE);
		print("# @ @ #  LumenOS -- ",WHITE); print("Version 0.4\n",CYAN);
		print("#     #\n",WHITE);
		print("# $$$ #  Kernel Version: ",WHITE); print("1.2SELFCOMP\n",YELLOW);
		print(" ##### \n",WHITE);
    }
	else if (strcmp(cmd, "color") == 0) print_info("Usage: color <color>!");
	else if (strncmp(cmd, "color ", 6) == 0) {
		char* color_arg = cmd + 6;
		if (strcmp(color_arg, "black") == 0) { change_color(BLACK); print_success("Text color changed!\n"); }
		else if (strcmp(color_arg, "blue") == 0) { change_color(BLUE); print_success("Text color changed!\n"); }
		else if (strcmp(color_arg, "green") == 0) { change_color(GREEN); print_success("Text color changed!\n"); }
		else if (strcmp(color_arg, "cyan") == 0) { change_color(CYAN); print_success("Text color changed!\n"); }
		else if (strcmp(color_arg, "red") == 0) { change_color(RED); print_success("Text color changed!\n"); }
		else if (strcmp(color_arg, "magenta") == 0) { change_color(MAGENTA); print_success("Text color changed!\n"); }
		else if (strcmp(color_arg, "brown") == 0) { change_color(BROWN); print_success("Text color changed!\n"); }
		else if (strcmp(color_arg, "gray") == 0) { change_color(GRAY); print_success("Text color changed!\n"); }
		else if (strcmp(color_arg, "dark_gray") == 0 || strcmp(color_arg, "dark gray") == 0) { change_color(DARK_GRAY); print_success("Text color changed!\n"); }
		else if (strcmp(color_arg, "light_blue") == 0 || strcmp(color_arg, "light blue") == 0) { change_color(LIGHT_BLUE); print_success("Text color changed!\n"); }
		else if (strcmp(color_arg, "light_green") == 0 || strcmp(color_arg, "light green") == 0) { change_color(LIGHT_GREEN); print_success("Text color changed!\n"); }
		else if (strcmp(color_arg, "light_cyan") == 0 || strcmp(color_arg, "light cyan") == 0) { change_color(LIGHT_CYAN); print_success("Text color changed!\n"); }
		else if (strcmp(color_arg, "light_red") == 0 || strcmp(color_arg, "light red") == 0) { change_color(LIGHT_RED); print_success("Text color changed!\n"); }
		else if (strcmp(color_arg, "light_magenta") == 0 || strcmp(color_arg, "light magenta") == 0) { change_color(LIGHT_MAGENTA); print_success("Text color changed!\n"); }
		else if (strcmp(color_arg, "yellow") == 0) { change_color(YELLOW); print_success("Text color changed!\n"); }
		else if (strcmp(color_arg, "white") == 0) { change_color(WHITE); print_success("Text color changed!\n"); }
		else if (strcmp(color_arg, "orange") == 0) { change_color(ORANGE); print_success("Text color changed!\n"); }
		else if (strcmp(color_arg, "pink") == 0) { change_color(PINK); print_success("Text color changed!\n"); }
		else if (strcmp(color_arg, "purple") == 0) { change_color(PURPLE); print_success("Text color changed!\n"); }
		else if (strcmp(color_arg, "lavender") == 0) { change_color(LAVENDER); print_success("Text color changed!\n"); }
		else if (strcmp(color_arg, "teal") == 0) { change_color(TEAL); print_success("Text color changed!\n"); }
		else if (strcmp(color_arg, "forest_green") == 0 || strcmp(color_arg, "forest green") == 0) { change_color(FOREST_GREEN); print_success("Text color changed!\n"); }
		else if (strcmp(color_arg, "olive") == 0) { change_color(OLIVE); print_success("Text color changed!\n"); }
		else if (strcmp(color_arg, "gold") == 0) { change_color(GOLD); print_success("Text color changed!\n"); }
		else if (strcmp(color_arg, "silver") == 0) { change_color(SILVER); print_success("Text color changed!\n"); }
		else if (strcmp(color_arg, "bronze") == 0) { change_color(BRONZE); print_success("Text color changed!\n"); }
		else print_error("Invalid color");
	}
	else if (strcmp(cmd, "test") == 0) print_info("Usage: test <test name>!");
    else if (strncmp(cmd, "test ", 5) == 0) {
        if (strcmp(cmd + 5, "gfxtest") == 0) sgl_test();
		else if (strcmp(cmd + 5, "mmtest") == 0) test_memory_manager();
		else if (strcmp(cmd + 5, "sndtest") == 0) speaker_test();
		else if (strcmp(cmd + 5, "mttest") == 0) test_multitasking();
		else if (strcmp(cmd + 5, "pgtest 1") == 0) paging_run_tests();
		else if (strcmp(cmd + 5, "pgtest 2") == 0) paging_print_info();
		else if (strcmp(cmd + 5, "pgtest 3") == 0) paging_demo();
        else {
	...
	else if (strcmp(cmd, "settime") == 0) print_info("Usage: settime <timezone>!");
	...
	else if (strcmp(cmd, "copy") == 0) print_info("Usage: copy <text>!");
	...
	else if (strcmp(cmd, "calc") == 0) print_info("Usage: calc <expression>!");
	...
	else if (strcmp(cmd, "mkfile") == 0) print_info("Usage: mkfile <file>!");
	...
	else if (strcmp(cmd, "wrtfile") == 0) print_info("Usage: wrtfile <file>!");
	...
	else if (strcmp(cmd, "rdfile") == 0) print_info("Usage: rdfile <file>!");
	else if (strncmp(cmd, "rdfile ", 7) == 0) {
		fs_cat_file(cmd + 7, current_dir_id);
	}
	else if (strcmp(cmd, "delcont") == 0) print_info("Usage: delcont <file / dir>!");
	...
	else if (strcmp(cmd, "mkdir") == 0) print_info("Usage: mkdir <directory>!");
		...
		if (dirname[0] == '\0') {
			...
			if (invalid) {
				print("Directory name contains invalid characters (spaces or symbols)\n", RED);
				log_message("Dir name parsing error",LOG_ERROR);
			} 
			else {
	...
	else if (strcmp(cmd, "cd") == 0) print_info("Usage: cd <directory>!");
	...
	else if (strcmp(cmd, "rnfile") == 0) print_info("Usage: rnfile <oldname> <newname>");
	...
	else if (strcmp(cmd, "fndfile") == 0) print_info("Usage: fndfile <pattern> [-r]\n");
	...
	else if (strcmp(cmd, "chusr") == 0) print_info("Usage: chusr <new username>!");
	else if (strncmp(cmd, "chusr ", 6) == 0) {
		change_username(cmd + 6);
	}
	else if (strcmp(cmd, "export") == 0) print_info("Usage: export <var name> <var value>!");
        ...
        print("CPU Frequency:\n", WHITE);
        print("  Current: ", WHITE); print_dec(current, CYAN); print(" MHz\n", WHITE);
        print("  Max:     ", WHITE); print_dec(max, CYAN); print(" MHz\n", WHITE);
        print("  Min:     ", WHITE); print_dec(min, CYAN); print(" MHz\n", WHITE);
    }
		...
		if (strncmp(args, "show", 4) == 0) {
			...
			if (sscanf(args + 7, "%d %d %d %d %d %d", 
					  &line, &color, &align, &bold, &italic, &underline) == 6) {
	...
	else if (strcmp(cmd, "run") == 0) print_info("Usage: run <filename>!");
	else if (strncmp(cmd, "run ", 4) == 0) {
		exec_run(cmd + 4);
	}
	else if (strcmp(cmd, "mkbasic") == 0) print_info("Usage: mkbasic <filename> <basic code>!");
