...
void set_environment_var(const char* name, const char* value, bool isSilent) {
    if (strlen(name) >= MAX_VAR_NAME_LENGTH || strlen(value) >= MAX_VAR_VALUE_LENGTH) {
        if (!isSilent) { print_error("Variable name or value too long"); }
    ...
    for (int i = 0; i < env_var_count; i++) {
        if (strcmp(env_vars[i].name, name) == 0) {
            ...
            if (!isSilent) { print_success("Variable updated"); }
            return;
        }
    }
    if (env_var_count >= MAX_VARIABLES) {
        if (!isSilent) { print_error("Maximum number of variables reached"); }
    ...
    env_var_count++;
	if (!isSilent) { print_success("Variable set"); }
        ...
        if (text[text_pos] == '$' && text[text_pos + 1] != '\0') {
            if (strncmp(text + text_pos, "$(all)", 6) == 0) {
                ...
                if (env_var_count == 0) {
                    ...
                    for (int i = 0; i < env_var_count && output_pos < MAX_COMMAND_LENGTH - 1; i++) {
                        if (i > 0) {
                            output[output_pos++] = ' ';
                        }
